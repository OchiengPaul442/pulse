(function () {
  ...
  function surfaceFatalError(msg) {
    ...
    if (vscode) {
      try {
        vscode.postMessage({ type: "webviewError", payload: msg });
      } catch (_) {}
    }
  }
  ...
  try {
    ...
    var initialSummary = JSON.parse(
      document.getElementById("root").dataset.initialSummary || "null",
    );
    var D = function (id) {
    ...
    // DOM refs
    var statusBadge = D("statusBadge"),
      statusTxt = D("statusTxt");
    var btnSettings = D("btnSettings"),
      btnRefresh = D("btnRefresh"),
      btnNewChat = D("btnNewChat");
    ...
    var btnAddMcp = D("btnAddMcp"),
      btnReloadMcp = D("btnReloadMcp"),
      btnSaveMcp = D("btnSaveMcp");
    var btnOpenMcpSettings = D("btnOpenMcpSettings"),
      btnManageMcp = D("btnManageMcp");
    var mcpList = D("mcpList"),
      mcpCount = D("mcpCount");
    var homeView = D("homeView"),
      chatView = D("chatView");
    var btnBack = D("btnBack"),
      attachmentRow = D("attachmentRow");
    var sessionList = D("sessionList"),
      messages = D("messages");
    var editsBanner = D("editsBanner"),
      bannerTxt = D("bannerTxt");
    var btnApply = D("btnApply"),
      btnRevert = D("btnRevert");
    var chipMode = D("chipMode"),
      chipModel = D("chipModel");
    var taskInput = D("taskInput"),
      btnSend = D("btnSend"),
      btnAttach = D("btnAttach");
    var permBtn = D("permBtn"),
      permBtnIcon = D("permBtnIcon"),
      permBtnLabel = D("permBtnLabel");
    var scrollBtn = D("scrollBtn"),
      selfLearnToggle = D("selfLearnToggle");
    var learningBadge = D("learningBadge");
    var todoDrawer = D("todoDrawer"),
      todoDrawerList = D("todoDrawerList"),
      todoDrawerCount = D("todoDrawerCount");
    var filesDrawer = D("filesDrawer"),
      filesDrawerList = D("filesDrawerList"),
      filesDrawerCount = D("filesDrawerCount");
    ...
    function renderFilesDrawer(files) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        var statsSpan =
          ...
          (added > 0
            ? '<span class="file-stat-add">+' + added + "</span>"
            : "") +
          (removed > 0
    ...
    function renderConversationMode(mode) {
      ...
      if (popup)
        popup.querySelectorAll(".popup-opt").forEach(function (btn) {
    ...
    function openModelPopup() {
      closeAllPopups();
      var popup = D("modelPopup"),
        list = D("modelPopupList");
      ...
      if (!models.length) {
        ...
        models.forEach(function (m) {
          ...
          btn.addEventListener("click", function (e) {
            ...
            if (chipModel) {
              chipModel.textContent =
                m.name.split(":")[0].slice(0, 14) || "\u2013";
              chipModel.title = m.name;
            }
            vscode.postMessage({
              type: "setModel",
              payload: { role: "planner", model: m.name },
            });
            vscode.postMessage({
              type: "setModel",
              payload: { role: "editor", model: m.name },
            });
            vscode.postMessage({
              type: "setModel",
              payload: { role: "fast", model: m.name },
            });
            closeModelPopup();
          });
          ...
          badges.appendChild(
            createModelBadge(
              sourceLabelForModel(m),
              sourceBadgeClassForModel(m),
            ),
          );
          if (m.supportsVision)
    ...
    function resetThinkingPanel() {
      ...
      thinkingStartTime = null;
      stepsCollapsed = false;
      var panel = D("thinkingPanel"),
        title = D("thinkingTitle"),
        elapsed = D("thinkingElapsed"),
        list = D("stepsList");
      if (!panel) return;
      ...
      if (kind === "reasoning") {
        ...
        if (active) {
          ...
          if (rt) {
            ...
            var normalizedTail = current
              .slice(
                Math.max(0, current.length - normalizedIncoming.length - 4),
              )
              .replace(/\s+/g, " ")
              .trim();
            var isPlaceholder =
    ...
    function normalizeMcpServer(s) {
      ...
      else if (typeof (s && s.args) === "string")
        args = s.args
          .split(/\r?\n/)
          .map(function (a) {
            return a.trim();
      ...
      return {
        ...
        trust: String((s && s.trust) || "workspace"),
        transport: t,
    ...
    function renderMcpServers(list) {
      ...
      mcpServers.forEach(function (server, idx) {
        ...
        card.innerHTML =
          ...
          "</textarea>";
        var ts = card.querySelector('select[data-field="transport"]'),
          trs = card.querySelector('select[data-field="trust"]');
        var ep = card.querySelector('input[data-field="endpoint"]'),
          ar = card.querySelector('textarea[data-field="args"]');
        ...
        card._read = function () {
          return {
            id: String(
              card.querySelector('input[data-field="id"]').value || "",
            ).trim(),
    ...
    function collectMcpServers() {
      return Array.from(mcpList.querySelectorAll(".mcp-card"))
        .map(function (c) {
          return typeof c._read === "function" ? c._read() : null;
        })
        .filter(Boolean)
        .filter(function (s) {
    ...
    function renderAttachments(files) {
      ...
      if (
        ...
        !attachmentRow.querySelector(".img-preview")
      ) {
      ...
      for (var i = 0; i < visibleCount; i++) {
        (function (s) {
          ...
          d.innerHTML =
            ...
            esc(s.title || s.id) +
            '</span><div class="session-actions"><div class="session-meta"><span class="sitem-time">' +
            esc(relTime(s.updatedAt)) +
            '</span><span class="session-count">' +
            esc(String(s.messageCount || 0)) +
            ' msgs</span></div><button type="button" class="session-delete" title="Delete">&#128465;</button></div>';
          d.addEventListener("click", function () {
            if (s.id)
              vscode.postMessage({ type: "openSession", payload: s.id });
          });
          d.querySelector(".session-delete").addEventListener(
            "click",
            function (e) {
              e.preventDefault();
              e.stopPropagation();
    ...
    function renderSummary(s) {
      ...
      if (learningBadge) {
        ...
        learningBadge.textContent =
          "Learning " +
          Math.max(0, Math.min(100, Math.round(learningPct))) +
          "%";
    ...
    function updateTokenRing(pct) {
      ...
      var ring = D("tokenRing"),
        arc = D("tokenRingArc"),
        label = D("tokenRingPct");
    ...
    function sendTask() {
      ...
      vscode.postMessage({
        type: "runTask",
        payload: { objective: text, action: action, messageId: messageId },
      });
      // Clear image previews after sending
      if (attachmentRow) {
        var imgPreviews = attachmentRow.querySelectorAll(".img-preview");
        for (var ip = 0; ip < imgPreviews.length; ip++)
          imgPreviews[ip].parentNode.removeChild(imgPreviews[ip]);
      }
      resetComposeState();
    }
    ...
    on(taskInput, "input", function () {
      autoGrow(taskInput);
      if (!isBusy) btnSend.disabled = taskInput.value.trim().length === 0;
    });
    ...
    on(messages, "click", function (e) {
      var btn = e.target;
      if (!btn || !btn.classList || !btn.classList.contains("code-copy"))
        return;
      e.stopPropagation();
    ...
    // Self-learn toggle
    on(selfLearnToggle, "change", function () {
      vscode.postMessage({
        type: "setSelfLearn",
        ...
        imageFiles.forEach(function (imgFile) {
          ...
          reader.onload = function (ev) {
            ...
            if (typeof dataUrl === "string") {
              ...
              vscode.postMessage({
                type: "dropImage",
                payload: { name: imgFile.name, dataUrl: dataUrl },
              });
            }
          };
    ...
    renderToolConfig();
    on(D("btnToolsAll"), "click", function () {
      TOOL_DEFS.forEach(function (t) {
        enabledTools[t.id] = true;
      });
      renderToolConfig();
    ...
    on(D("btnToolsNone"), "click", function () {
      TOOL_DEFS.forEach(function (t) {
    ...
    (function () {
      var p = D("modePopup");
      if (!p) return;
      p.querySelectorAll(".popup-opt").forEach(function (btn) {
        btn.addEventListener("click", function (e) {
          e.stopPropagation();
    ...
    (function () {
      var p = D("permPopup");
      if (!p) return;
      p.querySelectorAll(".perm-opt").forEach(function (btn) {
        btn.addEventListener("click", function (e) {
          e.stopPropagation();
          var m = btn.dataset.perm;
          if (m) {
    ...
    on(btnSyncModels, "click", function () {
    ...
    on(personaSelect, "change", function () {
    ...
    on(btnReloadMcp, "click", function () {
      vscode.postMessage({ type: "reloadMcpServers" });
    });
    on(btnOpenMcpSettings, "click", function () {
      vscode.postMessage({ type: "configureMcpServers" });
    });
    on(btnManageMcp, "click", function () {
    ...
    window.addEventListener("message", function (event) {
      ...
      if (type === "taskResult") {
        ...
        if (!isCancelled) {
          ...
          if (payload && payload.todos && payload.todos.length)
            renderTodoDrawer(payload.todos);
          if (payload && payload.fileDiffs && payload.fileDiffs.length)
          ...
          var diffData =
            payload && payload.fileDiffs && payload.fileDiffs.length > 0
              ? payload.fileDiffs
              : null;
          var wasAutoApplied = Boolean(payload && payload.autoApplied);
          var shouldReplaceAgent =
            pendingRequest &&
            pendingRequest.action === "retry" &&
